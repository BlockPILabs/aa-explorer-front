<script setup lang="tsx">
  ...
  import { getTopPaymaster } from '@/api/modules/stat'
  ...
  async function getList() {
    try {
      loading.value = true
      const res = await getTopPaymaster(props.network, {
        perPage: perPage.value,
        page: page.value
      })
      total.value = res?.totalCount
      if (page.value === 1) {
        list.value = (res?.PaymasterDetails || []).map((item) => {
          item.visible = false
          return item
        })
      } else {
        const newList = (res?.PaymasterDetails || []).map((item) => {
          item.visible = false
          return item
        })
